...
# Featury které nejsou dostupné v čase predikce D+1
OBSERVED_COLS = [
...
def _parse_ceps_items(result, rename_map=None):
    """Parsuje ČEPS SOAP response → DataFrame na minutovém/15min indexu."""
    rows = []
...
def _fetch_ace_history(ceps_client, days=7) -> pd.Series:
    """ACE posledních N dní z ČEPS (QH = 15min), ffill pro chybějící ISP."""
    ...
    try:
        ...
        df = _parse_ceps_items(result, {"value1": "ace_MWh"})
        if df.empty or "ace_MWh" not in df.columns:
            return pd.Series(dtype=float, name="ace_MWh")
        s = df["ace_MWh"]
        # Doplň na pravidelný 15min grid (ČEPS má ~5min zpoždění)
        idx_full = pd.date_range(s.index.min(), now, freq="15min", tz="Europe/Prague")
...
def _safe_ceps(ceps_client, method, kwargs, rename_map=None,
               master_idx=None) -> pd.DataFrame:
    ...
    try:
        ...
        df = _parse_ceps_items(result, rename_map)
        if df.empty or master_idx is None:
            return df
...
def _safe_entsoe(fn, kwargs, master_idx, ffill=True) -> pd.DataFrame:
    ...
    try:
        ...
        raw = raw.resample("15min").last().ffill() if ffill \
            else raw.resample("15min").mean().interpolate(limit=2)
...
def fetch_live_data(ceps_client, entsoe_client) -> pd.DataFrame:
    """
    Stáhne posledních 7 dní dat + D+1 forecasts.
    Vrátí DataFrame na 15min gridu zarovnaném na ACE index.
    """
    ace = _fetch_ace_history(ceps_client)
    if ace.empty:
    ...
    svr = _safe_ceps(ceps_client, "AktivaceSVRvCR",
                     ...
                     rename_map={"value1": "aFRR_up_MW", "value2": "aFRR_dn_MW",
    ...
    load = _safe_ceps(ceps_client, "Load",
                      ...
                      rename_map={"value1": "load_actual_pumping_MW",
    ...
    cb = _safe_ceps(ceps_client, "CrossborderPowerFlows",
                    {"agregation": "QH", "function": "AVG", "version": "RT"},
                    rename_map=CB_MAP, master_idx=master_idx)
    if not cb.empty:
    ...
    gen = _safe_ceps(ceps_client, "Generation",
                     ...
                     rename_map=GEN_MAP, master_idx=master_idx)
    if not gen.empty:
        frames.append(gen)
    ...
    res = _safe_ceps(ceps_client, "GenerationRES",
                     {"agregation": "QH", "function": "AVG",
                      "version": "RT", "para1": "all"},
                     rename_map=RES_MAP, master_idx=master_idx)
    if not res.empty:
        frames.append(res)

    # ── ENTSO-E known future ─────────────────────────────────────
    # DAP D0 + D+1 (ffill přes den pokud ještě není D+1 aukce)
    dap_frames = []
    for day in [today, tomorrow]:
        try:
            raw = entsoe_client.query_day_ahead_prices(
                "CZ", start=day, end=day + timedelta(days=1)
            )
            if raw is not None and not raw.empty:
                ...
                dap_frames.append(s.reindex(idx_15, method="ffill").to_frame())
        except Exception:
            pass
    if dap_frames:
        dap = pd.concat(dap_frames).reindex(master_idx, method="ffill")
        frames.append(dap)

    end_fc = pd.Timestamp(tomorrow + timedelta(days=1), tz="Europe/Prague")

    lfc = _safe_entsoe(
        entsoe_client.query_load_forecast,
        {"country_code": "CZ", "start": today, "end": end_fc},
        master_idx=master_idx, ffill=True,
    )
    if not lfc.empty:
    ...
    try:
        imp = entsoe_client.query_imbalance_prices(
            "CZ",
            start=pd.Timestamp(now - timedelta(days=7), tz="Europe/Prague"),
            end=pd.Timestamp(now, tz="Europe/Prague"),
        )
        if imp is not None and not imp.empty:
            imp = imp.tz_convert("Europe/Prague")
            imp.columns = [f"imbal_price_{c.lower()}_EUR" for c in imp.columns]
            imp = imp.resample("15min").last().ffill().reindex(master_idx, method="ffill")
            frames.append(imp)
    except Exception:
        pass
...
def build_features(df: pd.DataFrame, feature_cols: list) -> pd.DataFrame:
    ...
    # Odstraň observed (nejsou dostupné v čase predikce)
...
def run_prediction(ceps_client, entsoe_client) -> Optional[pd.DataFrame]:
    """
    Hlavní funkce — volej ze Streamlitu při refreshi.
    Vrátí DataFrame: index=timestamp, sloupce=ace_pred_MWh, h, predicted_at
    """
    ...
    # Nejnovější řádek s kompletními lags
    ...
    X_now = valid.loc[[t_now], feature_cols].values  # (1, n_features)

    # Predikuj klíčové horizonty, interpoluj zbytek
    key_preds = {}
...
def load_latest_predictions() -> pd.DataFrame:
    """Načte nejnovější sadu predikcí."""
...
def prediction_trace(df_pred: pd.DataFrame):
    """Hotový Plotly trace pro graf v dashboardu."""
