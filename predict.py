...
def _fetch_ace_history(ceps_client, days=7) -> pd.Series:
    ...
    try:
        ...
        idx_full = pd.date_range(s.index.min(), now, freq="15min")
    ...
    for day in [today, tomorrow]:
        try:
            ...
            if raw is not None and not raw.empty:
                ...
                s_15 = s.resample("15min").last().ffill()
                dap_frames.append(s_15.to_frame())
        except Exception:
            pass
    if dap_frames:
...
def _save_predictions(df_new: pd.DataFrame):
    ...
    try:
        if os.path.exists(PREDICTIONS_PATH):
            ...
            cutoff   = pd.Timestamp.now(tz="Europe/Prague") - pd.Timedelta(hours=48)
