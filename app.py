...
from data.ceps import (
    fetch_ceps_imbalance, fetch_ceps_svr, fetch_ceps_imbalance_price,
    fetch_ceps_all, ceps,
)
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 🤖 Model ACE")

    if st.button("⬇️ Stáhnout data pro model", use_container_width=True, key="sb_model_fetch"):
        from data.entsoe import client as _ec
        from data.ceps import ceps as _cc
        from tab_model import (
            _fetch_ace, _fetch_svr, _fetch_load,
            _fetch_dap, _fetch_load_fc, _fetch_wind_solar,
        )
        _now = pd.Timestamp.now(tz="Europe/Prague")
        _sources = {}
        _errors  = {}
        with st.spinner("Stahuji ACE..."):
            _df = _fetch_ace(_cc, _now)
            _sources["ACE"] = _df
            if _df.empty: _errors["ACE"] = "chyba"
        with st.spinner("Stahuji SVR..."):
            _df = _fetch_svr(_cc, _now)
            _sources["SVR"] = _df
        with st.spinner("Stahuji zatížení..."):
            _df = _fetch_load(_cc, _now)
            _sources["Zatížení RT"] = _df
        with st.spinner("Stahuji DAP..."):
            _df = _fetch_dap(_ec, _now)
            _sources["DAP ceny"] = _df
        with st.spinner("Stahuji load fc..."):
            _df = _fetch_load_fc(_ec, _now)
            _sources["Load forecast"] = _df
        with st.spinner("Stahuji wind/solar..."):
            _df = _fetch_wind_solar(_ec, _now)
            _sources["Wind/Solar fc"] = _df
        st.session_state["model_data"]   = _sources
        st.session_state["model_now"]    = _now
        st.session_state["model_errors"] = _errors
        st.session_state.pop("df_pred_model", None)
        st.success("✅ Data stažena")

    _sb_data_ready = "model_data" in st.session_state
    if st.button(
        "🔮 Spustit predikci (+15–120 min)",
        use_container_width=True,
        key="sb_model_predict",
        disabled=not _sb_data_ready,
    ):
        from tab_model import run_prediction_from_data, load_model
        _models, _, _ = load_model()
        if _models is None:
            st.error("Model nenalezen v data/models/")
        else:
            with st.spinner("Počítám predikci..."):
                _df_pred = run_prediction_from_data(st.session_state["model_data"])
            if _df_pred is not None:
                st.success(
                    f"✅ {_df_pred['predicted_from'].iloc[0].strftime('%H:%M')}"
                    f" → {_df_pred.index.max().strftime('%H:%M')}"
                )

    if _sb_data_ready:
        st.caption(f"Data: {st.session_state['model_now'].strftime('%H:%M:%S')}")
    if "df_pred_model" in st.session_state:
        _p = st.session_state["df_pred_model"]
        st.caption(
            f"Predikce: {_p['predicted_from'].iloc[0].strftime('%H:%M')}"
            f" → {_p.index.max().strftime('%H:%M')}"
        )
    ...
    with st.expander("🔋 Nastavení baterie"):
        ...
        hold_enabled = st.checkbox(
            ...
            help="Baterie drží aktuální SoC místo nabíjení/vybíjení")
...
with st.spinner("Načítám data rezerv…"):
    try:
        ...
        reserves = dict(
            afrr_d_amt=pd.DataFrame(), afrr_d_pri=pd.DataFrame(),
...
if not show_gas:
    ...
    if n_new or len(changes.get("ended", set())) or not changes["changed_mw"].empty:
        ...
        parts   = []
        if n_new:   parts.append(f"🆕 <strong>{n_new} nových</strong> odstávek")
        if n_ended: parts.append(f"✅ <strong>{n_ended} ukončených</strong>")
        if n_chmw:  parts.append(f"⚡ <strong>{n_chmw} změn MW</strong>")
...
# ══════════════════════════════════════════════════════════════════
# POMOCNÁ FUNKCE: Graf odchylky s predikcí
# ══════════════════════════════════════════════════════════════════

def _fig_ceps_combined_with_pred(df_ceps_imbal, df_ceps_price, ceps_load_series,
                                  load_fc, now_ceps):
    """
    Volá fig_ceps_combined a přidá predikční křivku z modelu.
    Graf je oříznut na okno: 2h zpět až 2h dopředu od now.
    """
    from charts.imbalance import fig_ceps_combined as _orig_fig
    ...
    # Ořízni x-osu na rozsah: -2h až +2h od now
    x_start = (now_ceps - pd.Timedelta(hours=2)).isoformat()
    x_end   = (now_ceps + pd.Timedelta(hours=2)).isoformat()
    fig.update_xaxes(range=[x_start, x_end])

    # Anotace — čas predikce pokud existuje
    df_pred = st.session_state.get("df_pred_model")
    if df_pred is not None and not df_pred.empty:
        pred_from = df_pred["predicted_from"].iloc[0]
        fig.add_annotation(
            x=pred_from.isoformat(),
...
if not show_gas:
    ...
    with tab_dash:
        st.markdown(
            '<div class="section-title">Systémová odchylka + zatížení + cena odchylky — ČEPS</div>',
            unsafe_allow_html=True,
        )
        ...
        _load_col = (
            "Load including pumping [MW]" if "Load including pumping [MW]" in ceps_d["load"].columns
            else "Load [MW]" if "Load [MW]" in ceps_d["load"].columns
            else None
        )
        ceps_load_series = (ceps_d["load"][_load_col] if _load_col else pd.Series(dtype=float))

        # ── Graf odchylky s predikcí ──────────────────────────────
        df_pred_exists = st.session_state.get("df_pred_model") is not None
        if df_pred_exists:
            st.info(
                "🟣 Graf zobrazuje okno ±2h od aktuálního času. "
                "Fialová čárkovaná křivka = predikce XGBoost modelu (+15 až +120 min)."
            )

        st.plotly_chart(
            _fig_ceps_combined_with_pred(
                df_ceps_imbal, df_ceps_price, ceps_load_series, load_fc, now_ceps
            ),
            use_container_width=True,
        ...
        st.markdown(
            '<div class="section-title">Aktivace SVR v ČR — ČEPS (minutová)</div>',
            unsafe_allow_html=True,
        )
        df_svr = fetch_ceps_svr()
        st.plotly_chart(
            fig_ceps_svr(df_svr, now_ceps),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        ...
        with _bc3:
            benefit_eur_mwh = st.number_input(
                "Benefit zákazníka [EUR/MWh]", value=8.0,
        ...
        if not df_ceps_imbal.empty or not df_imbal.empty:
            _fig_bal = fig_balancing_strategy(df_imbal, _ema, _signal, threshold_mw, now)

            # Přidej predikci XGBoost do grafu balancing strategie
            try:
                from tab_model import get_prediction_trace
                _pred_trace = get_prediction_trace()
                if _pred_trace is not None:
                    _pred_trace.update(name="Predikce XGBoost", showlegend=True)
                    _fig_bal.add_trace(_pred_trace)
            except Exception:
                pass

            # Rozšiř x-osu: 6h zpět + 2h dopředu
            _x_start = (now - pd.Timedelta(hours=6)).isoformat()
            _x_end   = (now + pd.Timedelta(hours=2)).isoformat()
            _fig_bal.update_xaxes(range=[_x_start, _x_end])
        ...
        st.markdown('<div class="section-title">Ceny aktivace záložních rezerv</div>',
                    unsafe_allow_html=True)
        st.plotly_chart(
            fig_activation_prices(df_act, now),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        ...
        with c1:
            if gen_raw.empty:
                ...
                st.plotly_chart(
                    fig_generation_area(gen_raw, now),
                    use_container_width=True,
                    config={"displayModeBar": False},
                )
        with c2:
            st.plotly_chart(
                fig_generation_donut(gen_raw),
                use_container_width=True,
                config={"displayModeBar": False},
            )
        with c3:
    ...
    with tab_ceps:
        ...
        if not df_f.empty:
            ...
            c2.metric("Frekvence", f"{last_hz:.3f} Hz", delta=f"{last_hz-50:.3f} Hz")
    ...
    with tab_out:
        ...
        st.markdown(f'<div class="section-title">Výrobní jednotky (PU) — {n_pu} aktivních</div>',
                    unsafe_allow_html=True)
        st.plotly_chart(
            fig_outages_gantt(df_out, "PU", now, changes),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        ...
        st.markdown(f'<div class="section-title">Generační jednotky (GU) — {n_gu} aktivních</div>',
                    unsafe_allow_html=True)
        st.plotly_chart(
            fig_outages_gantt(df_out, "GU", now, changes),
            use_container_width=True,
            config={"displayModeBar": False},
        )
        ...
        with st.expander(
            f"📋 Detail změn  ·  {n_new} nových · {n_ended_tab} ukončených · {n_chmw_tab} změn MW",
            ...
            if not (n_new or n_ended_tab or n_chmw_tab):
                ...
                if n_new and not df_out.empty:
                    new_df = (
                        df_out[df_out[["unit_raw","outage_start","outage_end"]]
    ...
    with tab_dap:
        ...
        if not _prices_combined.empty:
            ...
            st.plotly_chart(
                fig_battery_strategy(_df_sim, _avg - cycle_cost/2, _avg + cycle_cost/2, _avg, now),
                use_container_width=True, config={"displayModeBar": False},
            )
        ...
        if not dg_key:
            ...
            with st.spinner("Načítám Delta Green…"):
                try:
                    ...
                    total_down = (float(last2["downBatteryPowerKW"]) +
                                  float(last2["downSolarCurtailmentPowerKW"]))
    ...
    with tab_model:
        try:
            from data.entsoe import client as _entsoe_client
        except ImportError:
            from entsoe import EntsoePandasClient
            from config import ENTSOE_TOKEN
            _entsoe_client = EntsoePandasClient(api_key=ENTSOE_TOKEN)
        render_model_tab(ceps, _entsoe_client)
    ...
    if pivot_gas.empty:
        ...
        with tab_bar:
            ...
            if df_hist.empty:
                ...
                df_f1 = df_hist[df_hist["countryLabel"].isin(sel_countries)] if sel_countries else df_hist
                ...
                df_f2 = df_f1[df_f1["directionKey"].isin(sel_directions)] if sel_directions else df_f1

                all_systems = sorted(df_f2["adjacentSystemsKey"].dropna().unique())
                sel_systems = st.multiselect("🔧 Systém", all_systems, default=[],
                                             key="gas_systems", help="Prázdný výběr = všechny systémy")
                df_f3 = df_f2[df_f2["adjacentSystemsKey"].isin(sel_systems)] if sel_systems else df_f2

                all_points = sorted(df_f3["pointsNames"].dropna().unique())
                sel_points = st.multiselect("📍 Bod", all_points, default=[],
                                            key="gas_points", help="Prázdný výběr = všechny body")
                df_f4 = df_f3[df_f3["pointsNames"].isin(sel_points)] if sel_points else df_f3
                ...
                with col_qd:
                    date_range = st.date_input(
                        "📆 Rozsah (časová osa)",
                        value=(
                            df_hist["date"].dt.tz_localize(None).max() - pd.Timedelta(days=365),
        ...
        with tab_season:
            ...
            if df_hist_s.empty:
                ...
                df_s1 = df_hist_s[df_hist_s["countryLabel"].isin(sel_countries_s)] if sel_countries_s else df_hist_s
