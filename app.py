...
if not show_gas:
    ...
    # ──────────── TAB 7: MODEL ───────────────────────────────────────
    with tab_model:
        # Potřebujeme ČEPS klienta — stáhneme z data.ceps
        try:
            from data.ceps import ceps as _ceps_client
            from data.entsoe import client as _entsoe_client
            render_model_tab(_ceps_client, _entsoe_client)
        except ImportError:
            # Fallback — vytvoř klienty přímo
            from zeep import Client as SoapClient
            from entsoe import EntsoePandasClient
            from config import ENTSOE_TOKEN
            _ceps_client   = SoapClient(wsdl="https://www.ceps.cz/_layouts/CepsData.asmx?WSDL")
