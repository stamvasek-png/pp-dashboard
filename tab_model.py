...
def get_prediction_trace():
    """Vrátí plotly trace predikce pro vložení do fig_ceps_combined (osa Y = MW)."""
...
def render_model_tab(ceps_client, entsoe_client):
    ...
    if fetch_clicked:
        ...
        st.success("✅ Data stažena — nyní klikni na Krok 2")
    ...
    if not _data_ready:
        st.caption("⬆️ Nejprve stáhni data kliknutím na Krok 1 výše.")

    predict_clicked = st.button(
        "🔮 Krok 2 — Spustit predikci (+15–120 min)",
        type="secondary",
        use_container_width=True,
        key="model_predict_btn",
        disabled=not _data_ready,
        help="Nejprve stáhni data kliknutím na Krok 1" if not _data_ready else "Spustí predikci na +15 až +120 min",
    )
