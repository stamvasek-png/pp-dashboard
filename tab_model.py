...
def _fetch_ace(ceps_client, now: pd.Timestamp) -> pd.DataFrame:
    # ── Historie 7 dní: QH (15min agregát) ───────────────────────
    # QH je vstup do modelu (lags jsou v MWh/15min)
    start_hist = now - pd.Timedelta(days=7)
    start_recent = now - pd.Timedelta(minutes=90)

    df_hist = pd.DataFrame()
    try:
        r = ceps_client.service.AktualniSystemovaOdchylkaCR(
            dateFrom=start_hist.replace(tzinfo=None),
            dateTo=start_recent.replace(tzinfo=None),
            agregation="QH",
            function="AVG",
        )
        df_hist = _parse_ceps(r, {"value1": "ace_MWh"})
    except Exception as e:
        st.warning(f"ACE historie: {e}")

    # ── Posledních 90 minut: MI (minutová) → agreguj na QH ───────
    # MI je aktuálnější (~2 min zpoždění vs ~15 min u QH)
    df_recent = pd.DataFrame()
    try:
        r2 = ceps_client.service.AktualniSystemovaOdchylkaCR(
            dateFrom=start_recent.replace(tzinfo=None),
            dateTo=now.replace(tzinfo=None),
            agregation="MI",
            function="AVG",
        )
        df_mi = _parse_ceps(r2, {"value1": "ace_MW_raw"})
        if not df_mi.empty:
            # Minutová data [MW] → průměr za 15min interval → MWh/15min
            df_recent = df_mi["ace_MW_raw"].resample("15min").mean().to_frame("ace_MWh")
            # Přidej aktuální minutový řádek s přesným časem (pro status panel)
            df_recent.attrs["last_mi_ts"] = df_mi.index.max()
            df_recent.attrs["last_mi_val"] = float(df_mi["ace_MW_raw"].iloc[-1])
    except Exception as e:
        st.warning(f"ACE minutová: {e}")

    # ── Slouč historii + aktuální ─────────────────────────────────
    frames = [f for f in [df_hist, df_recent] if not f.empty]
    if not frames:
        return pd.DataFrame()

    df = pd.concat(frames)
    df = df[~df.index.duplicated(keep="last")].sort_index()
    df["ace_MW"] = df["ace_MWh"] * 4  # přepočet pro zobrazení
    return df
...
def render_model_tab(ceps_client, entsoe_client):
    ...
    for name, meta in SOURCE_META.items():
        ...
        if df.empty:
            ...
            # ACE: použij minutový timestamp pokud je dostupný
            if name == "ACE" and hasattr(df, "attrs") and "last_mi_ts" in df.attrs:
                last_ts = df.attrs["last_mi_ts"]
                extra   = f" (MI: {df.attrs['last_mi_val']:+.1f} MW)"
            else:
            ...
            stav    = "✅ OK" if lag_min <= 5 else ("⚠️ zpoždění" if lag_min <= 20 else "🔴 staré")
            if meta["klic"]:
                stav = stav + " ⭐"
            status_rows.append({
                "Zdroj":         name,
                "Popis":         meta["popis"] + extra,
                "API":           meta["zdroj"],
                "Řádků":         len(df),
